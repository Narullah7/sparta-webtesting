...
class SeleniumDemoReg
  ...
  MARITAL_STATUS = "[single, divorced, married]" # id
  HOBBY_STATUS =  # values
  ...
  def get_first_name_field_value
    @chrome_driver.find_element(:id, FIRST_NAME_FIELD).attribute('class')
    # sleep 2
  end
  ...
  def get_last_name_field_value
    @chrome_driver.find_element(:id, LAST_NAME_FIELD).attribute('class')
  end
  ...
  def select_marital_option
    ...
    marital_array = ['married','single','divorced']
    x = marital_array[rand(0..2).floor]
    @chrome_driver.find_elements(:value, '#{x}').click
